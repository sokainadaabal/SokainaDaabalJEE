...
import ma.enset.ebankingbackend.security.config.JwtConfig;
    ...
    public void refreshToken(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (jwt_token != null && jwt_token.startsWith(JwtConfig.TOKEN_HEADER_PREFIX)) {//Authorization: Bearer xxx
            ...
            try {
                ...
                AppUser user = securityService.loadByUserName(username); // get the user using the username
                ...
                String jwtAccessToken = JWT.create() // create a new JWT with the username
                        ...
                        .withClaim("roles", user.getRole().stream().map(AppRole::getRoleName).collect(Collectors.toList())) // roles
